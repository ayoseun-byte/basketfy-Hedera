...
import { Wallet, TrendingUp, Activity, DollarSign, ArrowUpRight, ArrowDownLeft, Plus, X, ExternalLink, ChevronDown, PieChart, BarChart3, Clock, Zap, ArrowRight } from 'lucide-react';
        ...
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          {!isConnected ? (
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="px-5 py-2.5 bg-purple-500/20 border border-purple-500/30 rounded-lg text-white font-mono text-sm flex items-center gap-2">
          ...
          <div className="bg-gradient-to-br from-slate-900 to-purple-900 border border-purple-500/30 rounded-2xl p-8 max-w-md w-full relative">
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <div className="relative">
                  <select value={selectedToken} onChange={(e) => setSelectedToken(e.target.value)} className="w-full px-4 py-3 bg-purple-950/50 border border-purple-500/30 rounded-lg text-white appearance-none cursor-pointer focus:outline-none focus:border-purple-500">
                    ...
                    {tokens.map((token) => (<option key={token.id} value={token.id}>{token.logo} {token.name} (Balance: ${token.balance})</option>))}
                  </select>
                  <ChevronDown className="absolute right-3 top-1/2 transform -translate-y-1/2 w-5 h-5 text-purple-400 pointer-events-none" />
                </div>
              </div>
              ...
              <div className="flex gap-3 pt-2">
                <button onClick={() => setShowDepositModal(false)} className="flex-1 px-4 py-3 bg-purple-500/20 hover:bg-purple-500/30 text-white rounded-lg font-medium transition-all">Cancel</button>
